...
function setup(){
    createP('HUMAN WRIST MOVEMENT PLOTTER ( Try to get full body in the frame)');
...
function gotPoses(poses){
    ...
    if(poses.length>0){
        ...
        if(poses.length>1){
if(pose){
            //    x1 = poses[0].pose.rightWrist.x;
            //   y1 = poses[0].pose.rightWrist.y;
            //  x2 = poses[1].pose.rightWrist.x;
            // y2 = poses[1].pose.rightWrist.y;
        tempPoses=poses.length;
    // for(let i=0;i<poses.length;i++)
    // {
    //  linex[i] = poses[i].pose.nose.x;
    //  liney[i] = poses[i].pose.nose.y;
    // }

    for( gi;gi<poses.length;gi++)
    {
     linex[gi] = poses[gi].pose.rightWrist.x;
     liney[gi] = poses[gi].pose.rightWrist.y;
    }
